...
class AdminController extends Controller
{
    ...
    public function changePassword(Request $request)
    {
     ...
     public function admin()
     {
          ...
          $sql2 = "select count(id) as tong from orders where status = 'done'  ";
          $data['sodon'] = \DB::select($sql2)[0]->tong;
          $sql3 = "select sum(final_amount) as tong from orders where status = 'done'  ";
          $data['tongdon'] = \DB::select($sql3)[0]->tong ?? 0;
          $sql4 = "select count(id) as tong from products where status = 'active'  ";
          $data['sohang'] = \DB::select($sql4)[0]->tong;
