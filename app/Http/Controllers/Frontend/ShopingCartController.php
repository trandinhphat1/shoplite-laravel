    ...
    public function order(Request $request)
    {
        ...
        if( $user)
        {
            $this->validate($request,[
                'ship_id'=> 'required|numeric|min:1|exists:address_books,id',
                'invoice_id'=> 'required|numeric|min:1|exists:address_books,id',
            ], [
                'ship_id.required' => 'Vui lòng chọn địa chỉ giao hàng',
                'ship_id.min' => 'Vui lòng chọn địa chỉ giao hàng',
                'ship_id.exists' => 'Địa chỉ giao hàng không tồn tại',
                'invoice_id.required' => 'Vui lòng chọn địa chỉ nhận hóa đơn',
                'invoice_id.min' => 'Vui lòng chọn địa chỉ nhận hóa đơn',
                'invoice_id.exists' => 'Địa chỉ nhận hóa đơn không tồn tại',
            ]);

            $data['vendor_id'] = $user->id;
            $data['customer_id'] = $user->id;
            $data['shiptrans_id'] = $request->ship_id;  
            $data['paidtrans_id'] = $request->invoice_id;  
            ///save product detail ////////////
            ////average price///////////////////
            $sql  = "select c.quantity, d.* from (SELECT * from shoping_carts where user_id = "
            .$user->id.") as c left join products as d on c.product_id = d.id where d.status = 'active'  ";
