            ...
            <form method="post" action="{{route('product.store')}}">
                ...
                <div class="intro-y box p-5">
                    ...
                    <div class="mt-3">
                        ...
                        <textarea  class="form-control"  name="summary" id="editor1"  required>{{old('summary')}}</textarea>
                    </div>
                    ...
                    <div class="mt-3">
                        ...
                        <input id="price_in" name="price_in" type="number" class="form-control" value="{{old('price_in', 0)}}" required>
                    </div>
                    <div class="mt-3">
                        <label for="regular-form-1" class="form-label">Giá vốn trung bình <span class="text-red-600">*</span></label>
                        <input id="price_avg" name="price_avg" type="number" class="form-control" value="{{old('price_avg', 0)}}" required>
                    </div>
                    <div class="mt-3">
                        <label for="regular-form-1" class="form-label">Giá bán <span class="text-red-600">*</span></label>
                        <input id="price_out" name="price_out" type="number" class="form-control" value="{{old('price_out', 0)}}" required>
                    </div>
                    <div class="mt-3">
                        <label for="regular-form-1" class="form-label">Kích thước <span class="text-red-600">*</span></label>
                        <input id="size" name="size" type="text" class="form-control" placeholder=" " value="{{old('size')}}" required>
                    </div>
                    <div class="mt-3">
                        <label for="regular-form-1" class="form-label">Cân nặng <span class="text-red-600">*</span></label>
                        <input id="weight" name="weight" type="number" step="0.01" class="form-control" placeholder=" " value="{{old('weight')}}" required>
                    </div>
                    <div class="mt-3">
                        <label for="regular-form-1" class="form-label">Bảo hành <span class="text-red-600">*</span></label>
                        <input id="expired" name="expired" type="number" class="form-control" placeholder=" " value="{{old('expired')}}" required>
                    ...
                    <div class="mt-3">
                     ...
                     <select id="select-junk" name="tag_ids[]" multiple placeholder="Start Typing..." autocomplete="off">
                        @foreach ($tags as $tag )
                            <option value="{{$tag->id}}" {{in_array($tag->id, old('tag_ids', [])) ? 'selected' : ''}} >{{$tag->title}}</option>
                        @endforeach
                     </select>
